...
# -*- coding: utf-8 -*-
"""
Created on Thu Nov 28 13:33:05 2024
...
Rext = np.linspace(2,3.7,50) #Rayon extérieur en cm
Rint = np.linspace(1.7,3.4,50) #Rayon intérieur a 4mm de moins que Rext (en cm)
h = np.linspace(0.1,3,90) #hauteur en cm
h_base = np.linspace(0.1,2.2,30)
#masse_lest = np.linspace(0,100,100)
...
#Boucle sur le nombre dans un vecteur
for N in range(0,51,1):
    masse_lest = N * (rho_bille * (4/3) * np.pi * 0.15**3 - rho * (4/3) * np.pi * 0.15**3)
    lest.append(masse_lest)
    ...
    for j in range(int(len(h))) :
        for d in range(int(len(h_base))) :
            for f in range(int(len(Rint))) :
                for b in range(len(lest)) :
                    if Rint[f] < Rext[i] and Rext[i]-Rint[f]>0.3:
                        masse_wheel = rho * np.pi * h[j] * (Rext[i]**2 - Rint[f]**2)
                        masse_base = rho * np.pi * h_base[d] * (Rext[i]**2)
                        masse_totale = masse_base + masse_wheel + lest[b]
                        I_cylindre_creux = (1/2) * (masse_wheel+lest[b]) * (Rext[i]**2 + Rint[f]**2)
                        I_cylindre_plein = (1/2) * masse_base * Rext[i]**2
                        I_wheel = I_cylindre_creux + I_cylindre_plein
                        h_tot = h[j] + h_base[d]

                        if I_wheel <= 1400 and I_wheel >= 1100 and masse_totale <= 125 :
...
print('Final : Masse =', masse_finale,' ; R_ext =', ok_Rext[indice], ' ; R_int =', ok_Rint[indice], ' ; hauteur =', ok_h_tot[indice], ' ; H base =', ok_h_base[indice], ' ; H roue =', ok_h[indice],'; Nombre de billes =', ok_masse_lest[indice]/0.09500176184455532)
